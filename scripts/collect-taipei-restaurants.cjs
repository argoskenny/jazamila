...
const fetchPage = createPageFetcher({ cacheDir: CACHE_DIR, requestDelayMs: REQUEST_DELAY_MS });
...
function parseStore(id, html, district) {
  ...
  const popularFoods = extractPopularFoods(html, cleanText);
  const google = extractGoogleRating(html);
  const foodDiaryCount = extractFoodDiaryCount(html);
  const reviewExcerpt = extractReviewExcerpt(html, cleanText);
  const priceRange = estimatePriceRange(extractPriceValues(html, cleanText), cuisineTypes, intro);
...
async function collectCandidateIds(district) {
  ...
  const pages = await mapWithConcurrency(urls, fetchPage, CONCURRENCY);
...
async function collectSupplementIds(district, categoryUrls, existingIds) {
  ...
  for (const categoryUrl of categoryUrls) {
    ...
    const pages = await mapWithConcurrency(urls, async (url) => fetchPage(url), CONCURRENCY);

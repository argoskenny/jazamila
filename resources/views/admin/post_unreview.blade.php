...
<body ontouchstart="">
	<div class="container">
		...
		<div class="jumbotron">
			...
			<p>
				<form action="admin/post_unreview/1" method="get" name="form_res_newdata" role="form">
					...
					<a href="admin/post_unreview/1"><button type="button" class="btn btn-warning" name="clear" id="clear" >清除條件</button></a>
				</form>
			</p>
		</div>
		<div class="row">
			...
			if(!empty($restuarant))
			{
				...
				{?>
					<a class="list_a" href="admin/post_edit/{{ $post_data['id'].'?list=post_unreview&p='.$list_record }}">
						<div class="col-12-lg">
							<h4>{{ $post_data['id'] }}. {{ $post_data['post_name'] }}</h4>
							<p>{{ $post_data['post_area_num'].' - '.$post_data['post_tel_num'] }}</p>
							<p>{{ $post_data['post_region'].$post_data['post_address'] }}</p>
						</div>
					</a>
		<?php 	}
			} 

...
<body ontouchstart="">
	...
	<form action="listdata/0/{{ $url_type }}/{{ $url_maxmoney }}/{{ $url_minmoney }}/1" method="get" id="form_keyword" name="form_keyword">
		<div class="main">
			<div class="col-lg-12 main_title">餐廳列表</div>
			<div class="col-lg-12 main_text">{{ $main_text }}</div>
			<div class="container">
				<div class="input-group">
					
						<input type="text" id="search_keyword" name="search_keyword" class="form-control" onfocus="if(this.value=='請輸入關鍵字'){this.value=''; $('#search_keyword').css('color','black');}" onblur="if(this.value==''){this.value='請輸入關鍵字'; $('#search_keyword').css('color','#CCC');}" value="{{ $search_keyword_value }}" alt="請輸入關鍵字">
					...
					<div class="option_select">
						...
						<div class="btn-group foodwhere_region">
							<button type="button" id="show_option_region" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								{{ $region_show = ($url_region == 0) ? '都可以' : $config['region'][$url_region] }} <span class="caret"></span>
							</button>
						...
						<b>地區</b>
						<input type="hidden" id="foodwhere_section" name="foodwhere_section" value="{{ $url_section }}">
						<div class="btn-group foodwhere_section">
							<button type="button" id="show_option_section" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								{{ $section_show = ($url_section == 0) ? '全區' : $config['section'][$url_section] }} <span class="caret"></span>
							</button>
					...
					<div class="option_select">
						...
						<div class="btn-group foodmoney_min">
							<button type="button" id="show_option_min" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								{{ $url_minmoney = ($url_minmoney == 0) ? '0元' : $url_minmoney.'元左右' }} <span class="caret"></span>
							</button>
							<ul class="dropdown-menu" role="menu">
						...
						<div class="btn-group foodmoney_max">
							<button type="button" id="show_option_max" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								{{ $url_maxmoney = ($url_maxmoney == 0) ? '無上限' : $url_maxmoney.'元左右' }} <span class="caret"></span>
							</button>
							<ul class="dropdown-menu" role="menu">
					...
					<div class="option_select">
						...
						<div class="btn-group foodtype">
							<button type="button" id="show_option_type" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								{{ $foodtype_show = ($url_type == 0) ? '都可以' : $config['foodtype'][$url_type] }} <span class="caret"></span>
							</button>
							<ul class="dropdown-menu" role="menu">
								<li><a href="javascript:void(0);" a_type="0">都可以</a></li>
								<?php
					...
					if(!empty($restuarant))
					{
						...
						{	$res_list_data = ($i%2==0) ? "res_list_data1" : "res_list_data2";?>
							<div class="col-lg-12 {{ $res_list_data }}">
								...
								<div class="flex_text">
									<ul>
										<li><b><a class="td_a" href="detail/<?php echo $res_data['id'].$list_record }}">{{ $res_data['res_name'] }}</a></b></li>
										<li><?php 
										...
										?></li>
										<li>{{ $res_data['res_region'].$res_data['res_section'].$res_data['res_address'] }}</li>
									</ul>
									<ul>
										<li><span>類型：</span>{{ $res_data['res_foodtype'] }}</a></li>
										<li><span>均價：</span>{{ $res_price = ($res_data['res_price'] != 0) ? $res_data['res_price'].' 元' : '未提供均價' }}</li>
									</ul>
								</div>
                                <div class="res_note">
                                	{{ $res_data['res_note'] }}
